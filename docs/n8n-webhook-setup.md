...
## 🧭 نسبة المحادثة إلى التاجر — `account_external_id`

المحادثة تخصّ **حساب الأعمال** الذي استلمها، ومنه يُعرف التاجر (الترحيل ٠١٩،
دالة `resolve_conversation`). لذلك يحمل كل ما تكتبه n8n معرّف الحساب الذي
وصلت عبره الرسالة — الوارد والصادر معاً:

| المسار | أين يُضاف |
|---|---|
| البوت الحي (إدراج مباشر في `conversation_log`) | عمود `account_external_id` في كل صف |
| `POST /api/webhooks/bot` | الحقل `data.account_external_id` |

القيمة هي **نفسها** التي يكتبها التاجر في «معرّف الحساب لدى Meta» حين يربط حسابه:

| القناة | المعرّف | موضعه المعتاد في حدث Meta الوارد |
|---|---|---|
| واتساب | `phone_number_id` | `entry[].changes[].value.metadata.phone_number_id` |
| ماسنجر | معرّف الصفحة | `entry[].messaging[].recipient.id` |
| إنستغرام | معرّف الحساب المهني | `entry[].messaging[].recipient.id` |

والرسالة الصادرة من البوت تحمل معرّف الحساب نفسه الذي وصلت عبره الرسالة التي
تردّ عليها. ⚠️ تحقّق من الموضع في تنفيذ فعلي لمسارك (Executions) قبل الاعتماد
عليه، وقارن القيمة بما يظهر في صفحة حسابات التاجر حرفاً بحرف.

ما يحدث بحسب القيمة:

- **حساب مربوط ومعتمد من برق**: المحادثة لتاجر الحساب. وإن كانت للزبون
  محادثة قديمة بلا حساب **لدى التاجر نفسه**، تكمل فيها ولا تُفتح له محادثة
  ثانية (التبنّي، الترحيل ٠٢٠). محادثته القديمة لدى تاجر آخر تبقى لذلك التاجر.
- **حساب غير مربوط أو غير معتمد بعد**: محادثة بلا تاجر يراها فريق برق وحده،
  وحدث في `failed_events` (`resolve_conversation` / `account_routing`). تُلحق
  بالتاجر تلقائياً حين يعتمد مالك المنصة حسابه من `/admin`، لكن محادثةً منفصلة
  عن القديمة: الحساب غير المعتمد لا يتبنّى.
- **بلا معرّف**: القاعدة القديمة مؤقتاً، أي المتجر الأول. تكمل في محادثته مع
  الزبون على القناة نفسها ولو تبنّاها حساب، فالتحديث الجزئي للمسار لا يشطرها.
  يُلغى هذا الاستثناء في الترحيل ٠٢١ بعد تحديث المسار كله.

⚠️ **الترتيب:** يُربط الحساب ويُعتمد **قبل** أن يرسل المسار معرّفه، وإلا فتحت
رسائل الزبائن القدامى محادثات منفصلة بدل أن تكمل في محادثاتهم.

ورد الموظف من اللوحة يصل `N8N_WEBHOOK_URL` حاملاً `account_external_id`
للمحادثة (`null` لمحادثة من المسار القديم)، لترسل n8n من الرقم الصحيح.
...
```json
{
  ...
  "data": {
    ...
    "account_external_id": "<phone_number_id الذي استلم الرسالة>"
  }
}
```
